...
To assist with the setup process, run `bunx hardhat vars setup`. To set a particular value, such as a private key
variable, execute this:

```sh
bunx hardhat vars set PRIVATE_KEY
...
Deploy a new instance of the contract via a task:

```sh
bun run task:deploy --owner OWNER_ADDRESS --network polygon-mumbai
```
